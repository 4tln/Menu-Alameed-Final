...
/* سلايدر الهيدر: تبديل تلقائي، سحب، وحماية الحفظ العادي */
(() => {
  ...
  const startAutoPlay = () => {
    stopAutoPlay();
    timerId = window.setInterval(() => showSlide(activeIndex + 1),4000);
  };
  ...
  dots.forEach(dot => {
    dot.addEventListener("click",() => selectSlide(Number(dot.dataset.heroSlide || 0)));
  });
  ...
  ["contextmenu","dragstart","selectstart","auxclick"].forEach(eventName => {
    root.addEventListener(eventName,event => event.preventDefault(),{capture:true});
  });
