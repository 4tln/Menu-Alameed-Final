...
document.addEventListener("visibilitychange", () => {
  if(!document.hidden) updateStoreStatus();
});
  ...
  menuArea.innerHTML = sections.map(section => `
    <section class="${section.category === "العروض" ? "offers-section" : ""}">
      ${section.category === "العروض" ? `
        <div class="offer-photo" role="img" aria-label="صحن اللمة ضمن عرض العميد">
          <span class="offer-photo-label">عرض العميد</span>
        </div>
      ` : ""}
      <div class="items-grid">
        ${section.items.map(item => {
          const offerItem = isAlameedOffer(item.name);
          ...
          <article class="item-card ${popular ? "is-popular" : ""} ${offerItem ? "offer-item-card" : ""}">
            <div class="item-card-head">
              <div class="item-name">${offerItem ? "عرض العميد" : escapeHtml(item.name)}</div>
              ${offerItem ? '<span class="offer-card-badge">عرض خاص</span>' : popular ? '<span class="popular-badge">🔥 الأكثر طلبًا</span>' : ''}
            </div>
            ${offerItem ? `
              <div class="offer-components" aria-label="محتويات العرض">
                <span>8 أسياخ مشاوي دجاج</span>
                <span>نصف دجاج فحم</span>
                <span>نصف دجاج برست</span>
                <span>مقبلات</span>
                <span>رز</span>
              </div>
              <div class="offer-deposit-hint">
                <span>تأمين الصحن يُضاف تلقائيًا</span>
                <strong>${money(PLATE_DEPOSIT)}${sarIcon("sar-symbol-small")}</strong>
              </div>
            ` : ""}
            <div class="variant-grid">
              ${item.variants.map(variant => `
                <button type="button" class="variant-btn"
                  ...
                  <span>${offerItem ? "أضف العرض" : escapeHtml(variant.size)}</span>
