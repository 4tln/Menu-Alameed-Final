...
function paintBrowserChrome(theme){
  ...
  if(lightThemeColor) lightThemeColor.media = darkMode ? "not all" : "all";
  if(darkThemeColor) darkThemeColor.media = darkMode ? "all" : "not all";

  const activeThemeColor = darkMode ? darkThemeColor : lightThemeColor;
  if(activeThemeColor){
    activeThemeColor.setAttribute("content", browserColor);
    document.head.appendChild(activeThemeColor);
  }

  const statusBarMeta = document.getElementById("appleStatusBarMeta");
  if(statusBarMeta){
    statusBarMeta.setAttribute("content", darkMode ? "black-translucent" : "default");
    document.head.appendChild(statusBarMeta);
  }

  /* Safari sometimes delays repainting its top chrome until a layout frame. */
